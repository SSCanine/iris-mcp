...
def run_self_test() -> dict:
    ...
    from iris.spatial import enumerate_windows, match_window, list_monitors, get_monitor_for_window, _make_window_info
    from iris.semantic import HAS_UIA, query, supports_uia, walk_tree
    from iris.vision import capture, ocr_text, _TESSERACT_OK
    from iris.tokens import FocusToken, revalidate, inspect
    from iris import resolver as resolver_mod
    from iris.fingerprint import compute_fingerprint, compare
    ...
    try:
        ...
        token = FocusToken.create(
            ...
            title=info.title, monitor_index=monitor_idx, bounds=info.bounds,
        )
        ...
        if _TESSERACT_OK:
            ...
            results.append(CheckResult(name="vision_ocr", status="skip", reason="tesseract_missing"))
        ...
        if HAS_UIA:
            ...
            results.append(CheckResult(name="semantic_query_titlebar", status="skip", reason="uia_missing"))
            results.append(CheckResult(name="semantic_walk_tree", status="skip", reason="uia_missing"))
        ...
        if _TESSERACT_OK:
            ...
            results.append(CheckResult(name="resolver_ocr_fallback", status="skip", reason="tesseract_missing"))
        ...
        if HAS_UIA:
            ...
            results.append(CheckResult(name="fingerprint_drift", status="skip", reason="uia_missing"))
        ...
        if HAS_UIA:
            ...
            results.append(CheckResult(name="discover_panels_smoke", status="skip", reason="uia_missing"))
